...
concentracion =1*1e3 #[concentracion]= g/m^3 (1 g/l == 1e3 g/m^3) (Default = 10000 g/m^3)
...
for k in range(len(fnames_m)):
    ...
    if Analisis_de_Fourier == 1:
        ...
        # fig_fourier.savefig(os.path.join(output_dir,fnames_m[k]+'_Espectro.png'),dpi=200,facecolor='w')
        # fig2_fourier.savefig(os.path.join(output_dir,fnames_m[k]+'_Rec_impar.png'),dpi=200,facecolor='w')
    ...
    sar_integracion = mu_0*Area_ciclo*frec_final_m/(concentracion)  #[sar]=[N/A^2]*[A^2/m^2]*[1/s]*[m^3/g]=W/g
    SAR_int.append(sar_integracion)
...
if Ciclo_promedio:
    ...
    ax.plot(H_prom/1000,M_prom,'-',c='tab:red',label=f'Ciclo promedio ({Num_ciclos_m} ciclos)')
    ...
    plt.xlabel('H (kA/m)',fontsize=15)
    plt.ylabel('M (A/m)',fontsize=15)
    plt.title('   '+fecha_graf + f'   {N_armonicos_impares} arm impares',loc='left',fontsize=12,y=0.95)
    plt.suptitle('Ciclos de histéresis promedio',fontsize=18)
    plt.savefig(os.path.join(output_dir,os.path.commonprefix(list(fnames_m))+'_ciclo_promedio.png'),dpi=300,facecolor='w')
else:
    pass
